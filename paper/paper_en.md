...
The download is split in two because `katevasma.py` refuses more than five
files per run; the ten-pulse scripts read the raw rounds from
`dedomena_curby/`, the single-pulse ones the unpacked `curby_28297.npz` beside
themselves. Random seeds are fixed in the scripts: 4711 (exclusion map shuffles), 808 and

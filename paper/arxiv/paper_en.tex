...
Bob. Both are needed, because the map is computed per channel. The
tables of \S6.3 and \S6.4 report \(O_{\mathrm{A}}\) vs
\(S_{\mathrm{B}}\) and so use \(\alpha_{\mathrm{A}}\);
\(\alpha_{\mathrm{B}}\) enters the dashed curves of both panels of
Figure 3, the worst point of the single-pulse map
(\(1.62 \times 10^{-1}\), exponential future at \(\tau\) = 1), and the
linearity check of \S2.
...
\(\rightarrow\) \textbf{92.2\(\sigma\) (\(\sqrt{G}\))} for
\(O_{\mathrm{A}}\) vs \(S_{\mathrm{A}}\); \(3.576 \times 10^{-4}\), G =
7,437 \(\rightarrow\) 86.2\(\sigma\) for \(O_{\mathrm{B}}\) vs
\(S_{\mathrm{B}}\) \\
...
\caption{Mutual information between one party's outcome at trial \textit{i} and the other party's setting at trial i+k, over all 20,001 lags $|k| \le 10{,}000$, round 28297, for both cross-pair channels. Drawing every lag would let one pixel of width stand for about forty of them and show only their maximum, so the lags are binned in hundreds: the light line is the largest value in each bin, the dark line the median. The dashed red line is the Bonferroni threshold at family-wise $\alpha$ = 0.05 over 40,002 hypotheses, $1.13 \times 10^{-6}$ bits per trial; the largest single lag in either channel reaches 63\% of it. The strip above each panel carries the same-trial value of that same wing on its own scale, which is what the factor of 362 means. The right-hand panel is the histogram of all 20,001 values of $\sqrt{G}$ against the $\chi^2(1)$ curve the threshold assumes.}
\label{fig:1}
\end{figure}
...
\caption{Eberhard J in units of its Poisson error, against the lag between settings and outcomes, round 28297. The y axis is broken: the narrow strip carries the decoupled baseline near $-135 \sigma$, which is a property of the statistic rather than a result, and the main axes carry the region where the question lies. Left: |k| $\le$ 50, every lag in that range measured, so the points are joined; k = 0 is drawn separately, since joining it would put a 147$\sigma$ vertical through the break. Right: |k| > 50 on a symmetric-log axis, where the lags are sampled rather than exhaustive (\S6.2), so the points are left unjoined. The violation is confined to exactly one lag.}
\label{fig:2}
\end{figure}
...
\caption{Upper bound on the coupling strength $\varepsilon$ against kernel width $\tau$, for all four kernel shapes and both cross-pair channels. Left: the single pulse of \S6.3, round 28297. Right: the ten-pulse joint bound of \S6.4, which is the strongest statement these data support. Solid lines are $O_{\mathrm{A}}$ vs $S_{\mathrm{B}}$, dashed $O_{\mathrm{B}}$ vs $S_{\mathrm{A}}$; everything above a curve is excluded at family-wise $\alpha$ = 0.05. The bend above $\tau \approx$ 3,000 is the finite scan window truncating the kernel tails. The upper axis gives the upper-bound conversion of \S4.1 and applies to round 28297 only.}
\label{fig:3}
\end{figure}
...
\caption{Recovery of an injected signal of known strength. The horizontal axis is the injected coupling in units of the bound this analysis states for that kernel and $\tau$; the vertical axis is the matched-filter significance. Points are the mean over ten repetitions, error bars their standard deviation, and the six series are the three asymmetric kernels at $\tau$ = 30 and $\tau$ = 300. The dashed line is not a fit: its slope is fixed by the measured z at $\varepsilon$ = $\varepsilon_{\mathrm{excl}}$ and it is forced through the origin, so the other three levels are a prediction the points either meet or do not. The boxes give the number of runs detected out of 60 at each level. Nothing is found when nothing is injected, and a signal at twice the stated bound is found every time.}
\label{fig:4}
\end{figure}
...
These joint bounds are the right panel of \textbf{Figure 3}, plotted
against the single-pulse map of \S6.3 on the left.
...
resolves \\
\texttt{code/figures\_en.py} & Figures 1--4 \\
\end{longtable}
...
The download is split in two because \texttt{katevasma.py} refuses more
than five files per run; the ten-pulse scripts read the raw rounds from
\texttt{dedomena\_curby/}, the single-pulse ones the unpacked
\texttt{curby\_28297.npz} beside themselves. Random seeds are fixed in
the scripts: 4711 (exclusion map shuffles), 808 and 909 (injection
verification), 1707 (power curve), 2026 (systematic diagnostics), 3141
(mismatch), 5150 (joint bound), 0 and \textbar k\textbar+1 (per-lag
nulls). Results were produced with Python 3.14.6, NumPy 2.5.1, SciPy
1.18.0 and Matplotlib 3.11.1; the minimum supported versions are given

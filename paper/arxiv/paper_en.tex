...
\textbf{In cryptographic units.} The bits of \S6.1 convert directly into
a guessing advantage. Consider an adversary who tries to predict the
setting \(S_{\mathrm{B}}\)(i+k), a uniformly random bit, from Alice's
outcome \(O_{\mathrm{A}}\)(i) at some lag k. For a uniform binary S the
...
with I in bits. With I \textless{} \(1.13 \times 10^{-6}\) bits at every
lag, the bias is below \(1.25 \times 10^{-3}\), and the guessing
probability satisfies
\(P(\hat S = S) < 0.5 + \mathrm{bias}/2 = 0.50063\), at the family-wise
confidence of \S6.1. The constant was checked rather than copied: the
binary form above is the tight one for a uniform bit, an exact inversion
of the binary entropy through Fano's inequality,
\(H(S|O) \le h(P_{\mathrm{err}})\), gives \(1.2516 \times 10^{-3}\) as
well, agreeing with the Pinsker form to seven digits, and the inequality
was verified numerically over a grid of binary channels. This is the
quantity a leakage lemma would take as input; we do not carry the
argument through to a min-entropy statement, and the bound is stated per
lag, not jointly over lags.
...
10,000 independent frequencies per channel, and the Bonferroni threshold
on Exp(1) is \(z = \ln(m/0.05)\): z = 12.90 for the two channels of one
...
\item
  Oscillatory kernels are not covered by the map of \S6.3, whose
  1/\(\sqrt{Q}\) regularity is empirical and established only within the
  tested class of one-signed kernels; an oscillatory kernel has a
  different optimal filter shape entirely. They are covered separately,
  and by a different statistic, in \S6.8: a Lomb--Scargle scan of all
  ten pulses finds no frequency above threshold and bounds an
  oscillatory coupling of unit peak at \(\varepsilon\) \textless{}
  \(5.5 \times 10^{-4}\). What remains untested is the intermediate case
  of a kernel that is neither one-signed nor a single sinusoid.
\item
...
guessed with a bias above \(1.25 \times 10^{-3}\) (\S6.7). The

...
\caption{Eberhard J in units of its Poisson error, as a function of the lag between settings and outcomes, round 28297. Left: |k| $\le$ 50, linear, every lag in that range measured, so the points are joined. Right: the full scanned range on a symmetric-log axis, showing that the behaviour at |k| up to 10,000 is identical to that at $|k| = 1$; here the lags are sampled rather than exhaustive (\S6.2), so the points are left unjoined. The violation is confined to exactly one lag. Vector version: \texttt{figures/fig2\_j\_curve.pdf}.}
\label{fig:2}
\end{figure}
